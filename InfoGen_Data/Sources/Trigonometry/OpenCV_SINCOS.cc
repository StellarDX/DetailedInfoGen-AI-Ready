...
/*
    ...
    使用 gmpy2 计算。
 */
const double __SinCos128F_Table[132]
{
    ...
    0x1.00000000000000000000000000p0L, 0x0.00000000000000000000000000p-104L,
...
void __cdecl __CV_CHEBYSHEV_SINCOS(double _X, double* _SIN, double* _COS)
{
    double x = _X;
    double z, sin_l, cos_l_m1;
    int64_t ix;
    uint32_t tix, index;

    ix = std::bit_cast<uint64_t>(x);
    tix = ((uint64_t)ix) >> 32;

    uint64_t N = ((SINCOS_TABSIZE / 4) - 1) * 8;
    ...
    if (absx < SINCOS_BOUNDARY)
    {
        ...
        z = x * x;
        SINCOS_SETV(_SIN, SIN0 * x + (x * (z * (SIN1 + z * (SIN2 + z * (SIN3 + z * (SIN4 +
            z * (SIN5 + z * (SIN6 + z * (SIN7 + z * SIN8))))))))));
        SINCOS_SETV(_COS, COS0 + (z * (COS1 + z * (COS2 + z * (COS3 + z * (COS4 +
    ...
    */
    ...
    z = t * t;
    sin_l = t * (SSIN0 + z * (SSIN1 + z * (SSIN2 + z * (SSIN3 + z * (SSIN4 + z * SSIN5)))));
...
double __cdecl __CV_SIN128F_C64F(double _X)
{
...
double __cdecl __CV_COS128F_C64F(double _X)
{
    double absx = std::abs(_X);
    double cosx;
...
double __cdecl sind(double x) {return __CV_SIN128F_C64F(x);}
double __cdecl cosd(double x) {return __CV_COS128F_C64F(x);}
double __cdecl cscd(double x) {return 1. / __CV_SIN128F_C64F(x);}
double __cdecl secd(double x) {return 1. / __CV_COS128F_C64F(x);}
...
// 比较函数
void compare_sin(double x)
{
    double custom = __CV_SIN128F_C64F(x);
    double stdlib = std::sin(x * M_PI / 180.0); // 转换为弧度
