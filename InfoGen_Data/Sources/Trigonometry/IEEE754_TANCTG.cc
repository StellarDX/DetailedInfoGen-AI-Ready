/*
    ...
    这个值时正切函数的曲率将会变得非常离谱且无法用有理多项式去拟合了，此时就需要再次使用
    三角恒等变换反转到38.6以内再去计算。剩下的用帕德逼近就可以得到一个足够精确的值。
*/
...
double __cdecl __IEEE854_PADE_TANCTG(double _X, bool _Inv)
{
    if (std::abs(_X) > 45)
    {
    ...
    double x = _X, xhi = x;
    hx = std::bit_cast<uint64_t>(xhi) >> 32;
    lx = std::bit_cast<uint64_t>(xhi) & ~(uint32_t(0));
    ix = hx & 0x7fffffff;
    ...
    if (ix < 0x3c600000) // x < 2**-57
    {
        ...
        {
            if ((ix | lx) == 0 && _Inv) {return 1. / std::abs(x);}
    ...
    /*
        ...
        但这样会增加计算量。故此处以38.6为分界点，大于此分界点的x使用正切
    ...
    */
    ...
    // 由于系数大小非常夸张，这里不能用秦九韶写法，要不然会出现“大数吃小数”导致额外的误差。
    ...
    double
        ...
        x8 = x6 * x2,
        x10 = x8 * x2;
    //T = T0 + x2 * (T1 + x2 * (T2 + x2 * (T3 + x2 * T4)));
    T = T4 * x8 + T3 * x6 + T2 * x4 + T1 * x2 + T0;
    //U = U0 + x2 * (U1 + x2 * (U2 + x2 * (U3 + x2 * (U4 + x2))));
    U = U5 * x10 + U4 * x8 + U3 * x6 + U2 * x4 + U1 * x2 + U0;
    R = T / U;
    ...
    if (_X >= TANCTG_BOUNDARY)
    {
        double v = _Inv ? -1.0 : 1.0;
        w = v - 2.0 * (H1 * x - ((w * w) / (w + v) - r));
        w = _Inv ? -w : w;
        return (sign < 0) ? -w : w;
    }
    ...
    if (_Inv) // Inv置1时返回ctg(x)
    {
        ...
        double x1 = H1 * x;
        uint64_t u1i = std::bit_cast<uint64_t>(w);
        u1i &= uint64_t(~uint32_t(0)) << 32;
        double u1 = std::bit_cast<double>(u1i);
        double v = r - (u1 - x1);	// u1+v = r+x
        double z = 1.0 / w;
        uint64_t z1i = std::bit_cast<uint64_t>(z);
        z1i &= uint64_t(~uint32_t(0)) << 32;
        double z1 = std::bit_cast<double>(z1i);
        double s = 1.0 - z1 * u1;
        return z1 + z * (s - z1 * v);
    }
...
double __cdecl __IEEE854_TAN128F_C64F(double _X)
{
    double absx = std::abs(_X);
    ...
    /* tan(Inf or NaN) = NaN */
    else if (std::isinf(_X) || std::isnan(_X))
    {
        return std::numeric_limits<double>::quiet_NaN();
    }

    /* 将范围缩减到0-180 */
    double tx = std::fmod(absx, 180);
    // tan(x)
    if (tx <= 45)
    {
    ...
    {
        return neg * std::numeric_limits<double>::quiet_NaN();
    }
    // tan(x) = -ctg(x - 90)
...
double __cdecl __IEEE854_CTG128F_C64F(double _X)
{
    double absx = std::abs(_X);
    int neg = std::signbit(_X) ? -1 : 1;
...
double __cdecl tand(double x) {return __IEEE854_TAN128F_C64F(x);}
double __cdecl cotd(double x) {return __IEEE854_CTG128F_C64F(x);}

}

#if 0 // tan/ctg Test program generated by Deepseek
#include <iostream>
#include <iomanip>
#include <cmath>
#include <limits>
...
// 辅助函数，用于打印比较结果
void compare_results(double x, const std::string& func_name, double custom, double stdlib)
{
    double abs_error = abs(custom - stdlib);
    double ulp_error = abs_error / std::numeric_limits<double>::epsilon();

    std::cout << std::fixed << std::setprecision(16);
    std::cout << func_name << "(" << x << "):" << '\n';
    std::cout << "  Custom: " << custom << '\n';
    std::cout << "  Stdlib: " << stdlib << '\n';
    std::cout << std::scientific << std::setprecision(3);
    std::cout << "  Abs error: " << abs_error << '\n';
    std::cout << "  ULP error: " << ulp_error << " ulp" << '\n';
    std::cout << '\n';
}
