...
CREATE TABLE ig_object (
    ...
    system_id        BIGINT NOT NULL REFERENCES ig_system (system_id) ON DELETE CASCADE,
    parent_object_id BIGINT REFERENCES ig_object (object_id) ON DELETE CASCADE,  -- 根节点为NULL
...
CREATE TABLE ig_identifiers (
    object_id BIGINT NOT NULL REFERENCES ig_object (object_id) ON DELETE CASCADE,
    alias     VARCHAR(255) NOT NULL,
    PRIMARY KEY (object_id, alias)
);
CREATE INDEX ix_ig_identifiers_alias ON ig_identifiers (alias);

CREATE TABLE ig_physical (
    object_id                    BIGINT PRIMARY KEY REFERENCES ig_object (object_id) ON DELETE CASCADE,
