...
from InfoGen_Data import CreateNamespace
...
def Register(MainArgParser):
    ...
    SysParser.add_argument("-i", "--store", action='store_true', help = "将输出的文档向量化后保存到ChromaDB（需要模型支持，仅Markdown输出时可用）")
    SysParser.add_argument("-n", "--namespace", type = str, default = "", help = "文档存储的命名空间（不存在会报错，命名空间可使用python InfoGen.py create namespace NAME创建）")
    # 创建命名空间
    NSParser = RCParser.add_parser("namespace", help = "创建命名空间")
    NSParser.add_argument("value", help = "命名空间名称")
