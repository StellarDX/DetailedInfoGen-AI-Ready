...
def Register(MainArgParser):
    ...
    SysParser.add_argument("--code-page", type = int, default = 65001, help = "输入的文件的编码（防止烫屯锟斤拷）")
    ...
    SysParser.add_argument("--sort-system", action='store_true', help = "对输出的内容按半长轴的顺序排序")
    ...
    SysParser.add_argument("--store", action='store_true', help = "将输出的文档向量化后保存到ChromaDB（需要模型支持，仅Markdown输出时可用）")
    SysParser.add_argument("--model-config", type = str, help = "向量化文本模型的配置文件")
    # 创建命名空间
